...
all state-action pairs continue to be updated\cite{sutton}. The full algorithm
of Q Learning is presented below.
...
\begin{algorithm}
      \caption{DQN learn(episodes, \(\alpha, \epsilon, \gamma\))}
      \label{alg:dqn_learn}
      \begin{algorithmic}[1]
            \State hello
      \end{algorithmic}
...
\begin{algorithm}
      \caption{DQN train(episodes, \(\alpha, \epsilon, \gamma\))}
      \label{alg:dqn_train}
      \begin{algorithmic}[1]
            \State hello
      \end{algorithmic}
...
\subsection{DQN}
To build our agent for DQN, a few more steps are necessary as we need a deep
neural network to approximate our Q values. Since DQN is a off policy
reinforcement learning algorithm, it uses a technique called experience replay.
Thus, our first step is to implement \verb+ReplayMemory+, which acts as a
replay buffer of past experiences. Each memory consist of a 5-tuple, namely
(state, action, reward, next state, done). We also implemented the
\verb+sample+ method since the algorithm learns by sampling from the buffer,
and updates the Q network accordingly.
...
exploration. Finally, the agent learns following Algorithm \ref{alg:dqn_learn}
and trains following Algorithm \ref{alg:dqn_train}.
