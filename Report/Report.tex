...
\begin{algorithm}
      ...
      \begin{algorithmic}[1]
            ...
            \begin{cases}
                  ...
                  r + \gamma \max_a \title{Q}(\text{next state}, a; \tilde{\theta}) & \quad \text{otherwise}
            \end{cases}\)
            \State Perform a gradient descent step on \(\left(y_t - Q(\text{state}, a; \theta)\right)^2\) with respect to the policy network parameters \(\theta\)
            \If {\(t \text{ mod } C = 0\)}
...
The core of the training lies in the lines 13 to 19, where the agent selects a
small batch of tuple randomly and learns from it using a gradient descent
update step.
